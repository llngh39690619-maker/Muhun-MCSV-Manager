...
## 1.2.9-beta.24 — 版本載入器補齊與探索篩選排列修正版

- CurseForge 檔案缺少載入器標籤時，客戶端會先顯示版本，再背景讀取該精確檔案的 manifest，更新下拉選單與右側資訊；不再借用同 Minecraft 版本其他檔案的 loader。
- 預覽先取得官方 CDN 的 HEAD 轉址再讀取 ZIP 片段，修正入口 CDN 對 Range 請求回覆 404 的情況；每個預覽上限 8 MiB／128 次 Range 請求，不下載整包內容，不取代實際安裝的完整檔案雜湊驗證。
...
- 模組、材質包與光影包下載已嵌入客戶端主畫面，不再開啟另一個下載視窗；搜尋按鈕不再被裁切，遊戲版本、載入器與排序維持固定寬度，「瀏覽全部」會回到模組包首頁並重設篩選。
- 模組包預設來源改為 CurseForge；API Key 的安全匯入、狀態與清除入口移至右上角一般設定，秘密仍只由目前 Windows 使用者的 DPAPI 保護檔案讀取，不寫入程式碼、EXE、一般設定或日誌。
- 版本顯示依內容種類使用結構化語意：模組包為「模組包名稱＋模組包版本／MC／載入器」，模組為「模組名稱＋模組版本／MC／載入器」，材質包與光影包只顯示自身名稱、版本與相容 MC，絕不套用無意義的載入器欄位。
- 版本文字會移除 `.zip`、`.mrpack`、`.jar`、重複名稱、Release 字樣與檔名式雜訊；Modrinth、CurseForge 與 FTB 統一保留模組包名稱，缺少實際遊戲版本的搜尋卡不再顯示「多版本」。
- 模組包與內容安裝不再提供沒有作用的畫面記憶體滑桿；安裝工作改用全域客戶端預設值。左側下載清單會依目前頁面操作正確佇列並顯示對應數量，工作名稱也不再重複專案名稱。
- 本版本只建立本機驗收安裝包，不建立 GitHub Release、不推送 tag，也不附加 Windows EXE、APK、簽章、雜湊或其他二進位成品至公開發布。
- 本機正式發行驗證通過：11 個測試專案共 `3,457 / 3,457` 項通過；Release 建置 `0` 警告、`0` 錯誤；26 個專案的 NuGet 弱點稽核為 `0` advisories；正式套件 manifest 驗證 `30` 個檔案。
- Android 驗證成品使用 `versionCode 41`，簽署 APK SHA-256 為 `51DAD693740F5DDA5F7247DC50D4680413B605C286F01E16AF77A8DB45D20231`。單一 Windows 安裝器大小 `259,854,328` bytes，SHA-256 為 `F9AA9ECB0C0F05776F1BD1DF414AB8BCB44BAA940F1BA3938F1A752090CF11FC`；本機 Authenticode 與 DigiCert 時間戳記驗證為 `Valid`。
- **狀態：Beta，本機驗證完成，未發布。**

## 1.2.9-beta.21 — 客戶端內容探索工作區版

- 客戶端模組包探索頁依已確認的 C 內容探索版重建：左側集中「內容探索」與「實用工具」，中央使用雙欄緊湊模組包卡片，右側提供所選模組包的圖片、版本、相容性、記憶體與安裝操作。
- Modrinth 與 CurseForge 改用官方總筆數及真實 offset 分頁，支援上一頁、指定頁與下一頁；FTB API 不支援 offset 時不顯示假分頁。
- 搜尋或切換來源、遊戲版本、載入器與分類時會回到第一頁，載入完成自動選取第一個項目並更新右側預覽。
- 內容探索頁的滑鼠滾輪可由卡片、側欄與空白區域向上傳遞到整頁捲動；底部下載列可在零工作時展開空狀態，也可呈現即時縮圖、進度與取消操作。
- Java Runtime 與遊戲設定分成獨立入口及內容；安裝工作在切頁或切換來源後仍會完成縮圖更新。
- 本版本只建立本機驗收安裝包，不建立 GitHub Release、不推送 tag，也不附加 Windows EXE、APK、簽章、雜湊或其他二進位成品。
- 本機正式發行驗證通過：11 個測試專案全部通過；Release 建置 `0` 警告、`0` 錯誤；26 個專案的 NuGet 弱點稽核為 `0` advisories；正式套件 manifest 驗證 `30` 個檔案。
- Android 驗證成品使用 `versionCode 40`，簽署 APK SHA-256 為 `67D282148E027FFE985E25F1AB3344FA4528ED0D970019F7E53DE9EFE7CA7DEF`。單一 Windows 安裝器大小 `259,846,584` bytes，SHA-256 為 `69EAC55AEB352ABD6A353A9BE081DBD54D309C7B8C3E037B1339DAE7B647A6E0`；本機 Authenticode 驗證為 `Valid`。
- **狀態：Beta，本機驗證完成，未發布。**

## 1.2.9-beta.20 — 伺服端 C／客戶端 B 遊戲啟動器介面版

- 伺服端主畫面採用已確認的 C 版配置：精簡選取伺服器摘要，集中「伺服器操作」，把 CPU、記憶體、Java、連接埠與執行時間整理進可收折的「伺服器資訊」，遠端入口回到右上角工具列，並移除伺服端不需要的客戶端資料夾資訊。
- 客戶端主畫面採用已確認的 B 遊戲啟動器版：保留實例主卡與新增實例入口，下方以模組、精選截圖、Java／記憶體三張資訊卡呈現實際資料，右側集中實例位置、來源、建立時間與最後啟動資訊。
- 下載清單改為可完整收折的右側欄；收折後主內容會取回可用寬度，欄內的下載項目清單仍可獨立展開或收合，非首頁背景工作則以有界浮動面板顯示。
- 客戶端摘要改由實例 registry、實際模組目錄、Java 選擇、記憶體設定與 JVM 參數投影；切換實例時的首頁模組更新與內容頁更新彼此隔離，避免同一實例的取消權杖競爭造成資料延遲或空白。
- 新增伺服端／客戶端版面、命令、在地化對稱與首頁資料投影測試；繁體中文與英文資源同步。
- 本機正式發行驗證通過：11 個測試專案共 `3405 / 3405` 通過；Release 建置 `0` 警告、`0` 錯誤；26 個專案的 NuGet 弱點稽核為 `0` advisories；正式套件 manifest 驗證 `30` 個檔案。
- Android 驗證成品使用 `versionCode 39`，簽署 APK SHA-256 為 `7C3CB202D13FE114661F6552B6E6E2C969DCE931B629EBEBFD55D735BE7EC8E6`。單一 Windows 安裝器大小 `259,838,760` bytes，SHA-256 為 `13BB9A438BB6A47B79C1D3596AA2D44E4E56FBB2C7598638E3D0E6B3820E0FB7`；本機自簽 Authenticode 驗證為 `Valid`，並具 DigiCert RFC 3161 時間戳記。
- 發行政策：1.2.9-beta.20 的 GitHub Pre-release 只提供 GitHub 自動產生的原始碼封存，不附加 Windows EXE、APK、簽章、雜湊或其他二進位成品。
- **狀態：Beta。**
