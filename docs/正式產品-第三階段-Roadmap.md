# X MCSV 1.1.0：第三最終階段 17 項驗收

更新日期：2026-08-28

## 正式產品邊界

X MCSV 1.1.0 的 Server 管理採 Windows Service 唯一寫入者架構。Server 程序、Port、Console、備份、模組包更新、遠端帳號、權限、通知、Provider 與產品更新都由 Service 持有；Windows GUI、Web、iOS PWA 與 Android APK 是同一套授權 API 的客戶端。互動式 Minecraft Java 客戶端則在目前登入的 Windows 使用者 Session 中執行，使用隔離實例、DPAPI 帳號 Vault 與可恢復的 PID／啟動時間／Java 路徑三重程序身分，不取得 Service 權限。
...
6. 受管理的 Minecraft Java 客戶端只能列出 Mojang 正式 release；下載、Loader、Java、模組包與圖片皆須限制官方 host、禁止 redirect、驗證 hash／格式／大小，且不得把 Microsoft token 或第三方 API Key 寫入 EXE、設定或日誌。基岩版只能交給固定的 Microsoft 官方 Launcher／Store 入口，不得偽裝成 Java 實例或自行鏡像遊戲檔。
...
| 3 | Windows GUI | WPF 只以具 ACL 的 Named Pipe IPC API 1.5 操作 Server Service；另提供目前使用者權限的隔離 Minecraft Java 客戶端工作區、正式版本／Loader 建立、啟動、內容與設定，以及基岩版 Microsoft 官方入口交接 | 協定握手不相容時 fail closed、Service 離線不回退成 GUI 直控 Server Java；Java 客戶端程序身分、帳號、Java 與內容操作各自 fail closed；基岩版不建立受管理實例；主要命令與狀態端到端測試 |
...
| 10 | 線上模組包目錄 | Server 目錄支援 Modrinth／CurseForge BYOK／FTB；客戶端支援 Modrinth 驗證安裝與 FTB 官方 App 協定交接；來源、排序、版本、Loader、分類、預覽圖與背景工作連動 | 每個篩選映射 Provider 查詢；快取、圖片解碼、下載與掃描有界；缺圖一致降級；客戶端不爬取 CurseForge 或 FTB；安裝後圖示與來源身分持久化 |
...
| 16 | 效能與穩定性 | Console 100 ms 批次、有界 queue、單次 Reset、玩家 lazy-load/cancel、presence 合併、資源 latest-only；客戶端日誌批次、內容整體掃描額度、圖片三工人下載與磁碟淘汰；成功背景工作自動清除 | 大量 Console／玩家／狀態／客戶端檔案不逐筆塞 UI Dispatcher、記憶體與磁碟快取有界、切換 Server／客戶端不阻塞、取消與關閉安全收斂 |
| 17 | 安裝、遷移、簽章與正式驗收 | locked restore、warnings-as-errors、十一個測試專案、self-contained win-x64、Authenticode、RSA-PSS、Provider ECDSA、版本化安裝及回復；發行目錄由簽署腳本安裝，不宣稱 portable EXE | 全新／升級／回復腳本驗證、私鑰在 repo 外、正式 manifest/ZIP/APK 全部重驗、SHA-256 報告、不可修改歷史使用中資料；只雙擊解壓後 EXE 不視為有效安裝 |
...
- `dotnet restore --locked-mode`、Release solution build 及十一個測試專案全部成功，且 warning/error 為零。十一個專案為 App、Client、Contracts、Core、Data、GameClient、Notifications、ProviderHost、Remote、Service 與 Updater Tests；HostileProvider 只是一個測試 fixture，不另計為測試專案。
