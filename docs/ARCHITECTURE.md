...
├─ Providers            Paper/Velocity、Mojang、Fabric/Forge/NeoForge、Mohist/Arclight/CatServer/Akarin、Spigot BuildTools、受管理 MinGit、Adoptium、FTB、Modrinth、CurseForge BYOK、經驗證的下載與受控官方 Installer
└─ Runtime
...
23. Server 線上模組包 production surface 允許 FTB、Modrinth 與 CurseForge BYOK。CurseForge 憑證只可由目前 Windows 使用者提供，並以單次唯讀 `SecureString` 傳入 provider；Server 使用者明確選擇保存，或客戶端成功讀取一次性設定檔時，才可用 DPAPI CurrentUser 密文寫入共用的每使用者 `ClientSecrets`。客戶端不得顯示 API Key 輸入框；成功匯入後必須清除明文設定檔，失敗不得覆寫原有憑證。Key 不得進入 EXE、repository、一般設定、日誌、URI、命令列、環境變數、背景工作定義、CDN request 或 Service IPC，取得的副本在作業後必須釋放。客戶端可透過官方 API 搜尋、查看及安裝正式版本；每個專案、根檔和相依檔都必須在下載當下重驗身分、可用性、散布政策、長度與最強官方雜湊，Loader 以 manifest primary loader 為提交權威。`latestFilesIndexes.modLoader` 只供下載前的保守顯示，歧義時不得猜測。只有根檔的官方散布政策或空 download-url 可切換官方專案頁，安全或完整性失敗不得降級為網頁備援，且不得爬取、繞過授權或重新託管檔案。
