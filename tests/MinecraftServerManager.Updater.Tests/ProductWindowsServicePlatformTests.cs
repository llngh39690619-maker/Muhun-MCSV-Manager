...
public sealed class ProductWindowsServicePlatformTests : IDisposable
{
    ...
    {
        var (servicePath, dataRoot) = CreateManagedInputs();
        ...
        await platform.ConfigureAndRestartAsync(servicePath, dataRoot, CancellationToken.None);
    ...
    public async Task ConfigureAndRestart_RegistrationMismatchFailsBeforeStartingService()
    {
        ...
        await Assert.ThrowsAsync<InvalidDataException>(() =>
            platform.ConfigureAndRestartAsync(servicePath, dataRoot, CancellationToken.None));
    ...
    private (string ServicePath, string DataRoot) CreateManagedInputs()
    {
