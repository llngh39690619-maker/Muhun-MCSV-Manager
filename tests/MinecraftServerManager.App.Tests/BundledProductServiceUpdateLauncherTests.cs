...
public sealed class BundledProductServiceUpdateLauncherTests : IDisposable
{
    ...
    [Fact]
    public void PublisherTrust_CleanPcAllowsOnlyPinnedSelfSignedChainErrors()
    {
        Assert.True(WindowsProductPublisherVerifier.IsAcceptableTrustResult(0, false));
        Assert.True(WindowsProductPublisherVerifier.IsAcceptableTrustResult(
            unchecked((int)0x800B0109),
            true));
        Assert.True(WindowsProductPublisherVerifier.IsAcceptableTrustResult(
            unchecked((int)0x800B010A),
            true));
        Assert.False(WindowsProductPublisherVerifier.IsAcceptableTrustResult(
